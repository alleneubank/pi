...
Bash returns the OS PID and a durable output path for a background command. Use ordinary Bash commands such as `ps` and `kill` to inspect or terminate it, and Read or Bash to inspect retained output. While commands remain active, model requests receive bounded transient process context. Each command emits one bounded terminal notification with its PID, output path, exit code or signal, final output, and diagnostics; quiet waiting makes no model requests.
