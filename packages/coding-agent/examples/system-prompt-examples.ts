 ...
 *
 * Each example is a section with a YAML frontmatter block (title, when-to-use,
 * explainer) followed by the prompt itself in a fenced code block.
 *
 ...
 *
 * Set PI_EXAMPLE_CWD to override the fake working directory shown in the prompts.
 */
...
function renderScenario(scenario: Scenario, cwd: string): string {
	...
	const prompt = scenario.build(cwd);
...
function renderDocument(cwd: string): string {
	const header = frontmatter({
		...
		description:
			"Rendered examples of pi's coding-agent system prompt across different contexts, with the narration and usage in frontmatter.",
		"generated-by": "examples/system-prompt-examples.ts",
	});
	const sections = scenarios.map((scenario) => renderScenario(scenario, cwd));
...
function main(): void {
	...
	const scenarioSlug = scenarioIndex !== -1 ? args[scenarioIndex + 1] : undefined;
