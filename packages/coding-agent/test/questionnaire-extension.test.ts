...
describe("questionnaire example", () => {
	...
	it.each([false, true])("submits unanswered questions with a custom answer present: %s", async (answerFirst) => {
		const ui = await openQuestionnaire([targets, runtime]);
		if (answerFirst) ui.press(down, down, down, enter, "FreeBSD", enter);
		else ui.press(tab);
		ui.press(tab, enter);
	...
	it("preserves drafts across tabs and omits changed, unconfirmed answers on submission", async () => {
		...
		ui.press(tab, down, " ", tab, tab);
		expect(ui.completed()).toBe(false);
		expect(ui.render()).toContain("Unanswered: Targets");
		expect(ui.render()).toContain("Press enter to submit");
		ui.press(tab);
		...
		expect((await ui.result).details).toMatchObject({
			cancelled: false,
			answers: [{ id: "runtime", value: "node", label: "Node.js", wasCustom: false, index: 1 }],
		});
	});
