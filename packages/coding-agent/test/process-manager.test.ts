...
function createManager(
	...
	limits?: { maxProcesses?: number; maxRecords?: number; maxOutputBytes?: number },
): ProcessManager {
...
describe("ProcessManager", () => {
	it("returns the spawned OS PID and emits one bounded terminal event", async () => {
		...
		const retained = readFileSync(started.outputPath, "utf8");
		expect(retained).toContain("diagnostic");
		expect(events[0].output).toBeUndefined();
		expect(events[0].diagnostics).toBeUndefined();
		manager.dispose();
	});
	...
	it("transfers the foreground Bash process through the human action", async () => {
		...
		expect(events).toHaveLength(1);
		expect(events[0]).toMatchObject({ pid: started.pid, exitCode: 0 });
		expect(events[0].output).toBeUndefined();
		expect(readFileSync(started.outputPath, "utf8")).toContain("done");
		manager.dispose();
	});
	...
	it("finalizes after the shell exits even when a descendant inherits stdio", async () => {
		...
		expect(events).toHaveLength(1);
		expect(events[0].output).toBeUndefined();
		expect(readFileSync(started.outputPath, "utf8")).toContain("complete");
		manager.dispose();
	});
