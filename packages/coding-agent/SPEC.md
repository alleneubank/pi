...
- **REQ-BGP-008 — Terminal reporting:** A terminal notification contains the PID, output path, and exit code or termination signal. It does not include command output. A signal-terminated process is never described as having an unknown exit code. Concurrent completions schedule at most one pending wake-up.
...
- **2026-09-21, ratified:** The spawned OS PID is the background-process identity exposed to the model and human. Platform commands remain the only termination API and the only way to read retained output.
...
- **2026-09-21, ratified:** A terminal notification names the PID, status, and output path. It does not copy the log into the transcript. The model reads that file only if the result is still needed.
- **2026-09-21, ratified:** Interactive `/processes` displays owned runtime records in the chat and does not enter model context.
...
- [ ] One terminal notification reports normal exit and signal termination correctly without copying command output into the transcript.
