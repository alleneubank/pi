...
export function createShellToolDefinition(
	...
	return {
		name: config.name,
		label: config.label,
		description: `Execute a ${config.shellName} command in the current working directory. Returns stdout and stderr. Output is truncated to last ${DEFAULT_MAX_LINES} lines or ${DEFAULT_MAX_BYTES / 1024}KB (whichever is hit first). If truncated, full output is saved to a temp file. timeout is a hard deadline. runInBackground returns immediately with the OS PID, output path, and JSON status path. Completion statuses are batched into the next model request; they never start a turn or wake the agent. Read output explicitly when needed. Wait for dependent work before proceeding. Files last until record eviction or session disposal. Otherwise the command stays in the foreground until it exits.`,
