...
- **Runtime-local background processes** — Hand model Bash to the background with a PID, output path, and status path. Completion is recorded for explicit inspection and does not start a turn or wake the conversation. `/processes` shows owned records to the human. See [Background processes](docs/usage.md#run-a-background-process).
