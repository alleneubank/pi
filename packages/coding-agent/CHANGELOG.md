...
- **Runtime-local background processes** — Let Bash continue after a response with PID and output tracking, active context, completion notifications, and process-tree cleanup. See [Background Processes](README.md#background-processes).
...
- Added runtime-local background Bash processes with explicit or automatic yielding, PID and output-path reporting, active-process context, terminal completion notifications, process-tree cleanup, and a configurable Shift+Ctrl+B handoff. See [Background Processes](README.md#background-processes).
