...
describe("mid-prompt slash command autocomplete", () => {
	...
	for (const [name, key] of [
		...
		for (const leading of ["", "  ", "use "]) {
			it(`accepts a later-line skill with ${name} after ${JSON.stringify(leading)} without submitting`, async (t) => {
				...
				editor.handleInput("\x1b[A");
				editor.handleInput("\x01");
				for (let i = 0; i < leading.length; i++) editor.handleInput("\x1b[C");
				for (const ch of "/git") editor.handleInput(ch);
				...
				assert.strictEqual(editor.getText(), `context\n${leading}/skill:git-commit then\nafter`);
				assert.deepStrictEqual(editor.getCursor(), { line: 1, col: leading.length + 17 });
				assert.strictEqual(editor.isShowingAutocomplete(), false);

				editor.handleInput("\x1b[45;5u"); // Undo only the completion
				assert.strictEqual(editor.getText(), `context\n${leading}/git then\nafter`);
				assert.deepStrictEqual(editor.getCursor(), { line: 1, col: leading.length + 4 });
			});
		}
	}

	it("completes recall below a collapsed paste and separator without submitting", async (t) => {
		t.mock.timers.enable({ apis: ["setTimeout"] });
		const editor = new Editor(createTestTUI(), defaultEditorTheme);
		editor.setAutocompleteProvider(new CombinedAutocompleteProvider([{ name: "skill:recall" }], "/tmp"));
		const submissions: string[] = [];
		editor.onSubmit = (text) => submissions.push(text);
		const paste = `${"context\n".repeat(20)}last line`;
		editor.handleInput(`\x1b[200~${paste}\x1b[201~`);
		for (const ch of "\n\n---\n\n/recal") editor.handleInput(ch);
		assert.strictEqual(editor.getText(), "[paste #1 +21 lines]\n\n---\n\n/recal");
		t.mock.timers.tick(20);
		await flushAutocomplete();
		assert.strictEqual(editor.isShowingAutocomplete(), true);
		assert.match(stripVTControlCharacters(editor.render(100).join("\n")), /skill:recall/);

		editor.handleInput("\r");
		assert.deepStrictEqual(submissions, []);
		assert.strictEqual(editor.getText(), "[paste #1 +21 lines]\n\n---\n\n/skill:recall");
		assert.strictEqual(editor.getExpandedText(), `${paste}\n\n---\n\n/skill:recall`);
		assert.deepStrictEqual(editor.getCursor(), { line: 4, col: 13 });
	});

	it("keeps later-line action tokens literal while offering skills for a bare slash", async (t) => {
		t.mock.timers.enable({ apis: ["setTimeout"] });
		const editor = createEditorWithCommands();
		editor.setText("context\n");
		editor.handleInput("/");
		...
		assert.match(rendered, /skill:git-commit/);
		assert.doesNotMatch(rendered, /Switch models/);
	...
	it("cancels and reopens later-line skills, then closes after backspacing past the slash", async (t) => {
		t.mock.timers.enable({ apis: ["setTimeout"] });
		const editor = createEditorWithCommands();
		for (const ch of "context\n/git") editor.handleInput(ch);
		t.mock.timers.tick(20);
		await flushAutocomplete();
		assert.strictEqual(editor.isShowingAutocomplete(), true);
	...
	it("still submits first-line commands on Enter when the prompt has later lines", async (t) => {
		t.mock.timers.enable({ apis: ["setTimeout"] });
		const editor = createEditorWithCommands();
		const submissions: string[] = [];
		editor.onSubmit = (text) => submissions.push(text);
